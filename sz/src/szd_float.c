...
void decompressDataSeries_float_3D_random_access_with_blocked_regression(float** data, size_t r1, size_t r2, size_t r3, unsigned char* comp_data){
	...
	//sihuan debug
	//printf("decompress with random access\n");
	size_t dim0_offset = r2 * r3;
	size_t dim1_offset = r3;
	size_t num_elements = r1 * r2 * r3;
						...
						for(size_t ii=0; ii<block_size; ii++){
							for(size_t jj=0; jj<block_size; jj++){
								for(size_t kk=0; kk<block_size; kk++){
									...
									//sihuan added
									unsigned int tmp = *(unsigned int*)(block_data_pos);
							...
							for(size_t ii=0; ii<block_size; ii++){
								for(size_t jj=0; jj<block_size; jj++){
									for(size_t kk=0; kk<block_size; kk++){
										...
										//sihuan added
										unsigned int tmp = *(unsigned int*)(&data_pos[ii*block_dim0_offset + jj*block_dim1_offset + kk]);
										tmp_dec_sum += (unsigned long)tmp;
										//sihuan edit ends
...
void decompressDataSeries_float_3D_decompression_given_areas_with_blocked_regression(float** data, size_t r1, size_t r2, size_t r3, size_t s1, size_t s2, size_t s3, size_t e1, size_t e2, size_t e3, unsigned char* comp_data){
	...
	printf("decompress with random access how about this time\n");
	...
	//sihuan added
	//printf("reading decompressed data checksum\n");
	uint64_t * dec_sum_incmp = (uint64_t *)malloc(num_blocks * sizeof(uint64_t));
	int tmp_status;
	size_t tmp_nbEle;
	//char tmp_flpath[600] = "";
        //sprintf(tmp_flpath,"%s/decsum.sum", global_dir );
	//dec_sum_incmp = readUInt64Data("/home/sili/NYX_data/decsum.sum", &tmp_nbEle, &tmp_status);
	//dec_sum_incmp = readUInt64Data(global_dir, &tmp_nbEle, &tmp_status);
	//printf("reading %zu elements and first 10 of them are:\n", tmp_nbEle);
	//for (int i = 0; i < 10; i++){
	//	printf("%lu ", dec_sum_incmp[i]);
	//}
	//printf("\n");
	...
	comp_data_pos += sizeof(int) + tree_size;
	//sihuan
	memcpy(dec_sum_incmp, comp_data_pos, num_blocks*sizeof(uint64_t));
	comp_data_pos += num_blocks*sizeof(uint64_t);
		...
		for(size_t i=sx; i<ex; i++){
			for(size_t j=sy; j<ey; j++){
				...
				//sihuan edit ends
					...
					if (dec_sum_incmp[block_sum_id] != tmp_dec_sum){
						printf("decompression in block %lu is wrong, try reexecute\n", block_sum_id);
					}
					block_sum_id++;
					//printf("\n");
