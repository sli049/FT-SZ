...
void flip_a_bit(unsigned* a, unsigned bit_2_flip){
	if (bit_2_flip >= 32 || bit_2_flip < 0){
	//	cout << "flip bit beyond 32" << endl;
		return;
	}
	unsigned shifted = *a >> bit_2_flip;
	unsigned flipped = 0;
	if (shifted % 2 == 0)
		flipped = *a + (1 << bit_2_flip);
	else
		flipped = *a - (1 << bit_2_flip);
	*a = flipped;
}
...
unsigned char * SZ_compress_float_3D_MDQ_decompression_random_access_with_blocked_regression(float *oriData, size_t r1, size_t r2, size_t r3, double realPrecision, size_t * comp_size){
...
#if 1	//sihuan added
	unsigned long * input_block_sum = (unsigned long*) malloc(num_blocks * sizeof(unsigned long));
	unsigned long * input_block_isum = (unsigned long*) malloc(num_blocks * sizeof(unsigned long));
	unsigned long * dec_blcok_sum = (unsigned long*) malloc(num_blocks * sizeof(unsigned long));
	long int * type_block_sum = (long int*) malloc(num_blocks * sizeof(long int));
	long int * type_block_isum = (long int*) malloc(num_blocks * sizeof(long int));
	size_t block_sum_id = 0;
	size_t dup_err = 0;
	//printf("the number of blocks are: %zu\n", num_blocks);
#endif	//sihuan edit ends
	...
	size_t err_inj_loop = (size_t)rand()%num_blocks;
	size_t loop_cnt = 0;
	unsigned bigger_smaller = (unsigned)rand()%2;
	size_t mach_epsilon_cnt = 0;
	//printf("test\n");
	//printf("err_inj_loop is: %zu\n", err_inj_loop);
#endif	//sihuan edit ends
...
#if 1							//sihuan added
							unsigned int tmp = *(unsigned int*)(pred_buffer_pos);
							tmp_sum += (unsigned long)tmp;
							tmp_isum += (unsigned long)i_sum_i*(unsigned long)tmp;
							i_sum_i++;
#endif							//sihuan edit ends
...
#if 1				//sihuan added
				input_block_sum[block_sum_id] = tmp_sum;
				input_block_isum[block_sum_id] = tmp_isum;
				block_sum_id++;
#endif				//sihuan edit ends
...
#if 0					//sihuan added: computation error injection
					if (loop_cnt == err_inj_loop){
						if (bigger_smaller == 1) f = 2.0*f;
						else f = 0.5*f;
						printf("\ncoefficient computation error injected at block %zu\n", loop_cnt);
					}
					loop_cnt++;
#endif					//sihuan edit ends
	...
	//printf("\n loop_cnt is: %zu\n", loop_cnt);
	for (int i = 0; i < 10; i++){
	//	printf("%lu ", input_block_sum[i]);
	}
	//printf("\n");
	//sihuan edit ends

#if 0	//sihuan mem err inject
	printf("mem error injected: ");
	printf("from %f ", *(oriData+1));
	*(oriData + 1) = 562.78;
	printf("to %f\n", *(oriData+1));
#endif
	//sihuan edit ends;
#if 0	//sihuan random mem err inject
	srand(time(NULL));
	//size_t rand_inj = (size_t) rand()%num_elements;
	size_t rand_inj = (size_t) rand()%(6*6*6);
	printf("error injected at: %zu\n", rand_inj);
	unsigned int rand_bit = (unsigned int) rand()%32;
	unsigned int float2int = *(unsigned int*)(oriData + rand_inj);
	printf("random mem err inject in input array: from %f to ", *(oriData + rand_inj));
	flip_a_bit(&float2int, rand_bit);
	*(unsigned int*)(oriData + rand_inj) = float2int;
	printf("%f \n", *(oriData + rand_inj));
#endif	//sihuan edit ends
...
#if 0 //sihuan added
	//printf("force to make use_mean true\n");
	//use_mean = 1;
	if (use_mean) printf("use mean\n");
	else printf("not use mean\n");
#endif //sihuan edit ends
...
#if 0	//sihuan added: error injection to sampling
	err_inj_loop = (size_t)rand()%num_blocks;
	printf("\nsampling result changed at block ID: %zu\n", err_inj_loop);
	indicator[err_inj_loop] = !indicator[err_inj_loop];
#endif 	//sihuan edit ends
		...
		for(size_t i=0; i<num_x; i++){
			for(size_t j=0; j<num_y; j++){
				for(size_t k=0; k<num_z; k++){
					...
					//				printf("%lu ", tmp_sum);
					//}
					//if (tmp_sum != input_block_sum[block_sum_id]) printf(" input mem error detected at block: %zu \n", block_sum_id);
					if (tmp_sum != input_block_sum[block_sum_id]){
						printf("mem error detected\n");

						//printf("the old sum is: %lu\n", tmp_sum);
						//unsigned long new_tmp_sum = 0;
						...
						size_t err_loc = 0;
						if (input_block_isum[block_sum_id] > tmp_isum)
						err_loc = (input_block_isum[block_sum_id] - tmp_isum)/(input_block_sum[block_sum_id] - tmp_sum) - 1;
						else err_loc = (tmp_isum - input_block_isum[block_sum_id])/(tmp_sum - input_block_sum[block_sum_id]) - 1;
						printf("\nmemory error location is: %zu\n", err_loc);
						printf("isum diff: %lu sum diff: %lu\n", input_block_isum[block_sum_id] - tmp_isum, input_block_sum[block_sum_id] - tmp_sum);
						printf("old sum is: %lu, new sum is: %lu\n", input_block_sum[block_sum_id], tmp_sum);
						pred_buffer_pos = pred_buffer + pred_buffer_block_size*pred_buffer_block_size + pred_buffer_block_size + 1;
						unsigned int tmp_t = *(unsigned int*)(pred_buffer_pos + err_loc);
						unsigned long corrected = input_block_sum[block_sum_id] - tmp_sum + (unsigned long)tmp_t;
						printf("correct value from %f to ", *(pred_buffer_pos + err_loc));
						*(unsigned int*)(pred_buffer_pos + err_loc) = (unsigned int) corrected;
						printf("%f \n", *(pred_buffer_pos + err_loc));					}

#endif					//sihuan edit ends

#if 1					//sihuan added
					unsigned long tmp_dec_sum = 0;
					long int tmp_type_sum = 0;
					long int tmp_type_isum = 0;
					size_t i_sum_i = 1;
#endif					//sihuan edit ends
						...
						for(size_t ii=0; ii<block_size; ii++){
							for(size_t jj=0; jj<block_size; jj++){
								for(size_t kk=0; kk<block_size; kk++){
									...
									//******************** sihuan duplication:
									float pred_dup = reg_params_pos[params_offset_d] + reg_params_pos[params_offset_c] * kk + reg_params_pos[params_offset_b] * jj + reg_params_pos[0] * ii;
									...
									if (itvNum < intvCapacity){
										...
										pred = pred + 2 * (type[index] - intvRadius) * tmp_realPrecision;
										//**************** sihuan duplication
										pred_dup2 = 2 * (type[index] - intvRadius) * tmp_realPrecision + pred;
										//ganrantee comporession error against the case of machine-epsilon
										if(fabs(curData - pred)>tmp_realPrecision){
											//sihuan added
											mach_epsilon_cnt++;
...
#if 1									//sihuan added
									unsigned int tmp = *(unsigned int*)(&pred);
									tmp_dec_sum += (unsigned long)tmp;
									tmp_type_sum += (long int)type[index];
									tmp_type_isum += (long int)(i_sum_i * type[index]);
									i_sum_i++;
#endif									//sihuan edit ends

									//******************** sihuan test duplication:
									//if ((pred_dup - pred > 1e4) || (pred_dup2 - pred > 1e4)) dup_err++;
									if ((pred_dup + pred_dup2 - pred > 1e4)) dup_err++;
								...
								for(size_t kk=0; kk<block_size; kk++){
									...
									else
									{
										pred3D = cur_data_pos[-1] + cur_data_pos[-strip_dim1_offset]+ cur_data_pos[-strip_dim0_offset] - cur_data_pos[-strip_dim1_offset - 1]
												 - cur_data_pos[-strip_dim0_offset - 1] - cur_data_pos[-strip_dim0_offset - strip_dim1_offset] + cur_data_pos[-strip_dim0_offset - strip_dim1_offset - 1];
										//************************** sihuan duplication
										pred3D_dup = cur_data_pos[-strip_dim0_offset - strip_dim1_offset - 1] - cur_data_pos[-strip_dim0_offset - strip_dim1_offset] - cur_data_pos[-strip_dim0_offset - 1]
										 - cur_data_pos[-strip_dim1_offset - 1] + cur_data_pos[-strip_dim0_offset] + cur_data_pos[-strip_dim1_offset] + cur_data_pos[-1];
										diff = curData - pred3D;
										itvNum = fabs(diff)/realPrecision + 1;
										if (itvNum < intvCapacity_sz){
											if (diff < 0) itvNum = -itvNum;
											...
											*cur_data_pos = pred3D + 2 * (type[index] - intvRadius) * tmp_realPrecision;
											//********************* sihuan duplication
											dup2 = 2 * (type[index] - intvRadius) * tmp_realPrecision + pred3D;
											//ganrantee comporession error against the case of machine-epsilon
											if(fabs(curData - *cur_data_pos)>tmp_realPrecision){	
												//sihuan added
												mach_epsilon_cnt++;
...
#if 1 									//sihuan added
									unsigned int tmp = *(unsigned int*)(cur_data_pos);
									tmp_dec_sum += (unsigned long)tmp;
									tmp_type_sum += (long int)type[index];
									tmp_type_isum += (long int) (i_sum_i * type[index]);
									i_sum_i++;
#endif									//sihuan edit ends
									//*********************** sihuan test duplication
									//if ((pred3D_dup - pred3D > 1e4) || (dup2 - pred3D > 1e4)) dup_err++;
									if ((pred3D_dup + dup2 - pred3D > 1e4) ) dup_err++;
...
#if 1					//sihuan added
					type_block_sum[block_sum_id] = tmp_type_sum;
					type_block_isum[block_sum_id] = tmp_type_isum;
					dec_blcok_sum[block_sum_id++] = tmp_dec_sum;
#endif					//sihuan edit ends
...
#endif									//sihuan edit ends;
									//********************* sihuan test duplication
									//if (fabs(pred_dup - pred) > 1e4 * (fabs(pred) + 0.1)) printf("duplication failed....\n");
									//if (pred_dup == pred) dup_err++;
									//if ((pred_dup - pred > 1e4) || (pred_dup2 - pred)) dup_err++;
									if ((pred_dup + pred_dup2 - pred > 1e4)) dup_err++;
							...
							for(size_t jj=0; jj<block_size; jj++){
								for(size_t kk=0; kk<block_size; kk++){
									...
									pred3D = cur_data_pos[-1] + cur_data_pos[-strip_dim1_offset]+ cur_data_pos[-strip_dim0_offset] - cur_data_pos[-strip_dim1_offset - 1]
											 - cur_data_pos[-strip_dim0_offset - 1] - cur_data_pos[-strip_dim0_offset - strip_dim1_offset] + cur_data_pos[-strip_dim0_offset - strip_dim1_offset - 1];
									//************************* sihuan duplication
									float pred3D_dup = cur_data_pos[-strip_dim0_offset - strip_dim1_offset - 1] - cur_data_pos[-strip_dim0_offset - strip_dim1_offset]- cur_data_pos[-strip_dim0_offset - 1] - cur_data_pos[-strip_dim1_offset - 1] + cur_data_pos[-strip_dim0_offset] + cur_data_pos[-strip_dim1_offset] + cur_data_pos[-1];
									diff = curData - pred3D;
									itvNum = fabs(diff)/realPrecision + 1;
									if (itvNum < intvCapacity_sz){
										if (diff < 0) itvNum = -itvNum;
									...
									//****************** sihuan test duplication
									//if (fabs(pred3D_dup - pred3D) > 1e4 * (fabs(pred3D) + 0.1)) printf("duplication failed....\n");
									//if (pred3D_dup == pred3D) dup_err++;
									//if ((pred3D_dup - pred3D > 1e4) || (dup2 - pred3D > 1e4)) dup_err++;
									if ((pred3D_dup + dup2 - pred3D > 1e4)) dup_err++;
									index ++;
									cur_data_pos ++;
	...
	//sihuan added: write the dec block sum into file
	//char[100] tmp_targetpath;
	//sprintf(tmp_targetpath, "%s", "/Users/Li/NYX_data/dec_blcok_sum.sum");
	free(input_block_sum);
	free(input_block_isum);
	int tmp_status;
	//printf("\nstart writing dec_block_sum\n");
//	char tmp_flpath[600] = "";
//	sprintf(tmp_flpath,"%s/decsum.sum", global_dir );
	//writeULongData_inBytes((uint64_t *)dec_blcok_sum, num_blocks, "/home/sili/NYX_data/decsum.sum", &tmp_status);
	//writeULongData_inBytes((uint64_t *)dec_blcok_sum, num_blocks, global_dir, &tmp_status);
	
	//sihuan edit ends


#if 0	//sihuan debug
	printf("\nthe dec_blcok_sum is: \n");
	for (int i = 0; i < 10; i++){
		printf("%lu ", dec_blcok_sum[i]);
	}
	free(dec_blcok_sum);

	printf("\nthe type_block_sum is: \n");
	for (int i = 0; i < 10; i++){
		printf("%ld ", type_block_sum[i]);
	}
...
//	writeULongData_inBytes((uint64_t *)dec_blcok_sum, num_blocks, global_dir, &tmp_status);
	memcpy(result_pos, dec_blcok_sum, num_blocks*sizeof(uint64_t));
	result_pos+=num_blocks*sizeof(uint64_t);
	free(dec_blcok_sum);
	memcpy(result_pos, &use_mean, sizeof(unsigned char));
	result_pos += sizeof(unsigned char);
	memcpy(result_pos, &mean, sizeof(float));
...
#if 0	//sihuan inject type array error
	printf("inject type array error at 1 from %d to ", type[1]);
	type[1] = 345;
	printf("%d\n", type[1]);
#endif	//sihuan edit ends
...
#if 0 //sihuan random mem err inject
	srand(time(NULL));
	size_t rand_inj = (size_t) rand()%(num_blocks * max_num_block_elements);
	unsigned tmp4flip = *(unsigned *)(type + rand_inj);
	unsigned rand_bit = rand()%32;

	printf("inject type array at index: %zu from %d to ", rand_inj, *(type+rand_inj));
	flip_a_bit(&tmp4flip, rand_bit);
	*(unsigned *)(type + rand_inj) = tmp4flip;
	printf("%d\n", *(type+rand_inj));
...
#if 1				//sihuan added
				long int tmp_sum = 0;
				for (size_t t_i = 0; t_i < max_num_block_elements; t_i++){
					tmp_sum += (long int)type[t_i];
				}
				if (tmp_sum != type_block_sum[block_sum_id]){
					printf("\nmem err in type array\n");
					//calculate isum
					long int tmp_isum = 0;
					for (size_t t_i = 0; t_i < max_num_block_elements; t_i++){
						tmp_isum += (long int)((t_i+1) * type[t_i]);
					}
					size_t err_loc = (type_block_isum[block_sum_id] - tmp_isum)/(type_block_sum[block_sum_id] - tmp_sum) - 1;
					printf("\ntype array mem err location is: %zu\n", err_loc);
					printf("corrected from %d ", type[err_loc]);
					type[err_loc] = (int) (type_block_sum[block_sum_id] - tmp_sum + type[err_loc]);
					printf("to %d\n", type[err_loc]);
				}
	...
	//sihuan added: free type array sum
	free(type_block_sum);
	free(type_block_isum);
	//sihuan print duplication error
	printf("dup err: %zu\n", dup_err);
	printf("machine epsilon count: %zu\n", mach_epsilon_cnt);
