...
int main(int argc, char * argv[])
{
	...
	double nyx_rel_bound[6] = {1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4};
	// Hurricane
    ...
    // this needs to be changed according to your supercomputer's directory
	...
	size_t est_compressed_size = r1 * r2 * r3 * sizeof(float) * num_vars / 5.0;
...
//	num_vars = 1;
	...
	for(int i=0; i<num_vars; i++){
		...
		sprintf(global_dir, "%s/%d/decsum_%d", folder, folder_index, i);
		//sprintf(global_dir, "%s_%d", )
		// Read Input Data
		if(world_rank == 0){
			start = MPI_Wtime();
